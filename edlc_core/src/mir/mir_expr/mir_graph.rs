...
pub struct MirFlowGraph {
    ...
    /// These are shadow SSA values that basically only exist on the caller side.
    /// They are never actually generated for the function itself, however they are needed for a
    /// smooth analysis pipeline.
    pub caller_params: Vec<MirValue>,
}
...
impl MirFlowGraph {
    pub fn new<I: Iterator<Item=MirTypeId>>(
        ...
        for param in parameters.iter() {
            let ty = out.temp_vars[param.0].ty;
            out.temp_vars.push(TempVarData {
                ty,
            });
            let caller_value = MirValue(out.temp_vars.len() - 1);
            out.caller_params.push(caller_value);
            out.var_scopes.set(&caller_value, ValueScope::Function);
            out.var_scopes.set(param, ValueScope::Block(scope));
        }
    ...
    /// Parameters that correspond to the root blocks parameters.
    /// These parameters shadow the source values from the caller of a function; they dont't
    /// actually exist in the function body itself.
    /// However, due to the day the analysis pipeline works, these data sources need to be
    /// represented somehow.
    /// We just need to make sure that we don't map the caller shadow values to any actual stack
    /// or register space during SSA value deconstruction.
    pub fn get_caller_parameters(&self) -> &[MirValue] {
        &self.caller_params
    }

    /// If a value is a shadow value, the value does not actually exist on the stack frame
    /// generated by the graph.
    /// Instead, the value must live on the stack frame of one of its parents.
    pub fn is_shadow_value(&self, value: &MirValue) -> bool {
        self.caller_params.contains(value)
    }
    ...
    pub fn borrows(
        ...
        state.1.insert_func_parameters(&mut state.0);
